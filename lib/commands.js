...
const TouchPhase = { began: 1, moved: 2, ended: 3 }
...
async function openSession(creds, debug) {
  ...
  session.send({ _i: '_sessionStart', _t: 2, _x: txn++, _c: { _srvT: 'com.apple.tvremoteservices', _sid: sid } })
...
function sendAppLaunch(handle, bundleId) {
  handle.session.send({ _i: '_launchApplication', _t: 2, _x: handle.nextTxn(), _c: { _bundleID: bundleId } })
}

function sendTouchEvent(handle, x, y, phase) {
  handle.session.send({
    _i: '_touchUpdate',
    _t: 2,
    _x: handle.nextTxn(),
    _c: { _x: Math.round(x), _y: Math.round(y), _tP: phase, _tI: 1 }
  })
}
...
async function sleep(creds, debug) { await runHID(creds, HID.sleep, debug) }
async function playPause(creds, debug) { await runHID(creds, HID.playPause, debug) }
async function menu(creds, debug) { await runHID(creds, HID.menu, debug) }
async function back(creds, debug) { await runHID(creds, HID.back, debug) }
async function home(creds, debug) { await runHID(creds, HID.home, debug) }
...
async function volumeUp(creds, debug) { await runHID(creds, HID.volumeUp, debug) }
async function volumeDown(creds, debug) { await runHID(creds, HID.volumeDown, debug) }
async function up(creds, debug) { await runHID(creds, HID.up, debug) }
async function down(creds, debug) { await runHID(creds, HID.down, debug) }
async function left(creds, debug) { await runHID(creds, HID.left, debug) }
async function right(creds, debug) { await runHID(creds, HID.right, debug) }
async function click(creds, debug) { await runHID(creds, HID.click, debug) }
...
module.exports = {
  HID, TouchPhase,
  openSession, sendHID, sendTouchEvent, sendAppLaunch,
  sleep, playPause, menu, back, home, settings, volumeUp, volumeDown, up, down, left, right, click, wakeDevice
}
