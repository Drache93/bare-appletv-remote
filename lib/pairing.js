...
// opts.identity ({ clientId, ltsk, ltpk }) re-pairs with an existing identity
// so the ATV replaces our pairing record instead of accumulating a new peer.
async function pair(host, port, getPinFn, opts = {}) {
  const { debug = false, identity } = opts
  const conn = new CompanionConnection({ host, port, debug })
  try {
    return await runPairSetup(conn, getPinFn, identity, debug)
  } finally {
    await conn.close()
  }
}

async function runPairSetup(conn, getPinFn, identity, debug) {
  await conn.ready()
      ...
      try {
        ...
        if (state === 2) {
          ...
          if (identity) {
            ...
            ltsk = Buffer.from(identity.ltsk, 'hex')
            ltpk = Buffer.from(identity.ltpk, 'hex')
          } else {
