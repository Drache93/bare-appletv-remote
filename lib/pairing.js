...
async function pair(host, port, getPinFn, debug) {
  ...
  const creds = await new Promise((resolve, reject) => {
    ...
    conn.on('frame', async ({ payload }) => {
      try {
        ...
        if (state === 2) {
          ...
          conn.send(FrameType.PS_NEXT, opack.encode({ _pd: m3Data }))
        } else if (state === 4) {
          ...
          conn.send(FrameType.PS_NEXT, opack.encode({ _pd: m5Data }))
        } else if (state === 6) {
    ...
    conn.send(FrameType.PS_START, opack.encode({ _pd: m1Data }))
  })
