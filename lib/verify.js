...
async function verify(host, port, creds, debug) {
  ...
  const keys = await new Promise((resolve, reject) => {
    ...
    conn.on('frame', ({ payload }) => {
      try {
        ...
        if (state === 2) {
          ...
          // Derive session read/write keys (Companion protocol uses empty salt)
          const writeKey = hkdf(shared, Buffer.from(''), Buffer.from('ClientEncrypt-main'), 32)
          const readKey = hkdf(shared, Buffer.from(''), Buffer.from('ServerEncrypt-main'), 32)
    ...
    conn.send(FrameType.PV_START, opack.encode({ _pd: m1Data, _auTy: 4 }))
  })
