...
async function verify(host, port, creds, debug, timeoutMs = 10000) {
  const conn = new CompanionConnection({ host, port, debug: debug || false })
  try {
    return await runVerify(conn, creds, debug, timeoutMs)
  } catch (err) {
    await conn.close()
    throw err
  }
...
async function runVerify(conn, creds, debug, timeoutMs) {
  await conn.ready()
  ...
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error('Pair verify timed out')), timeoutMs)
  })
      ...
      conn.on('frame', ({ payload }) => {
        try {
          ...
          if (tlv[Tag.ERROR] !== undefined) {
            ...
            const names = { 2: 'Authentication', 3: 'Backoff', 4: 'MaxPeers', 6: 'Unavailable' }
            const label = names[code] ? `${names[code]} (${code})` : String(code)
            return reject(
              Object.assign(new Error('Pair verify failed: ' + label), { verifyError: code })
            )
          }
